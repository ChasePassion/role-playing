...
export default function ProfilePage() {
    ...
    const [sidebarCharacters, setSidebarCharacters] = useState<Character[]>([]); // separate list/logic for sidebar if needed
    ...
    const loadUserCharacters = async () => {
        ...
        try {
            ...
            // Also update sidebar list based on history or favorites? For now just use same list or empty
    ...
    return (
        <div className="flex h-screen overflow-hidden relative text-text-primary">
            {/* Overlay */}
            ...
            {/* Sidebar */}
            <div className={`shrink-0 transition-all duration-300 ease-in-out h-full overflow-hidden ${isOverlay ? "fixed left-0 top-0 z-50" : "relative"} ${isSidebarOpen ? "w-64" : "w-0"}`}>
                <Sidebar
